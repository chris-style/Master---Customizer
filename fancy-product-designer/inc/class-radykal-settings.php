	...
	class Radykal_Settings {
		...
		public function output() {
			...
			foreach( $current_tab_blocks as $key => $value ) {
				...
				echo '<div class="radykal-settings-block '.$active_class.'" id="'.$key.'">';
		...
		public static function output_item( $parameters ) {
			...
			//select
				...
				$input_html = '<select id="'.esc_attr( $id ).'" name="'.esc_attr( $id.$brackets ).'" style="'.esc_attr( $css ).'" class="'.esc_attr( $class ).'" '.$placeholder.' '.$multiple.' '.$dataSelected.'>';
